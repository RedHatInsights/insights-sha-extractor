...
from insights_sha_extractor.sha_extractor_error import SHAExtractorError
from insights_sha_extractor.schemas import INPUT_MESSAGE_SCHEMA, IDENTITY_SCHEMA
...
class Consumer(ICMConsumer):
    ...
    def deserialize(self, bytes_):
        ...
        if isinstance(bytes_, (str, bytes, bytearray)):
            try:
                ...
                return SHAExtractorError(f"Unable to decode received message: {ex}")

            except jsonschema.ValidationError as ex:
                return SHAExtractorError(f"Invalid input message JSON schema: {ex}")

            except binascii.Error as ex:
                return SHAExtractorError(
    ...
    def handles(self, input_msg):
        ...
        if isinstance(input_msg.value, SHAExtractorError):
            LOG.error(
